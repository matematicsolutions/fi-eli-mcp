...
Pulls a tiny static ``fi-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/fi-runtime.json
...
Opt-out: set ``FI_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "FI_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/fi-eli-mcp/releases/latest/download/fi-runtime.json.gz",
)
_CACHE_NAME = "fi-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "fi-eli-mcp-runtime (+https://github.com/matematicsolutions/fi-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("FI_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "fi-eli"
