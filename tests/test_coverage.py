...
from fi_eli_mcp.coverage import build_coverage
from fi_eli_mcp.server import fi_coverage


def _coverage():
    fn = getattr(fi_coverage, "fn", fi_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FI_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("fi_coverage" in line for line in lines), (
        "an audit entry was written but it does not name fi_coverage"
    )
